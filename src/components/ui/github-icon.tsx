...
/**
 * GitHub マークの inline SVG アイコン。
 * lucide-react v1 でブランドアイコン（`Github` 等）が削除されたため、自前で保持する。
 * `fill="currentColor"` により親の文字色（`hover:text-accent` 等の遷移）をそのまま継承する。
 * `size` は lucide の API 互換（footer の `<link.icon size={16} />` を変更せず差し替えられる）。
 * 装飾用途のため `aria-hidden`（ラベルはリンク側の `aria-label` が担う）。
 */
